...
# would look structurally near-zero regardless of model quality.
#
# Hallucination labels' "detail" text embeds the exact inserted sentence
# verbatim, so sentence-level checkers (AIChecker) substring-match fine there.
# But ConceptChecker/DeterministicChecker don't predict sentences at all --
# their (type, error) pairs are terse concept+polarity phrases like
# "haematuria (negated)", compared against a label whose "detail" is the full
# source sentence ("Brief episode of haematuria, now resolved."). That's a
# GRANULARITY mismatch (phrase vs. sentence), distinct from the omission
# register mismatch above, but it produces the same symptom: a real detection
# that never substring-matches. Confirmed via concrete cross-referenced
# evidence on prim10/prim11/prim13 (haematuria, lives-with-family, visual aura
# all showed up in both FP "added" and FN "missed" logs for the same file).
# So the embedding fallback applies to both prediction types now -- deliberately
# not using AlignScore/SummaC/FactKB themselves as the similarity judge, since
# that would bias the comparison toward whichever checker's own scoring style
# happens to resemble its output.
#
...
# sentences) -- recalibrate once real matching data is visible at scale. Reused
# as-is for hallucination-type's phrase-vs-sentence comparisons below; that's a
# different kind of mismatch than the omission register gap it was tuned on, so
# treat this threshold as a first pass for hallucination-type, not a calibrated one.
...
class Evaluate:
    ...
    def compare(self, errors, filename, elapsed=None, run=None):
        ...
        (case-insensitive substring) or their sentence embeddings are at least
        SIMILARITY_THRESHOLD cosine-similar (see module docstring above for why
        both omission and hallucination types need this fallback). Stores the
        ...
        for p_type, p_error in predicted:
            ...
            for i, (a_type, a_error) in enumerate(actual):
                if matched_actual[i]:
                    continue
                if p_type == a_type and self._texts_match(p_error, a_error):
    ...
    @staticmethod
    def _texts_match(p_error, a_error):
